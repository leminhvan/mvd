...
class Prefs_model extends CI_Model {
    ...
    public function user_info_login($id)
    {
        ...
        if ($user != NULL) {
            ...
            $data['sinhnhat']    = htmlspecialchars($user->sinhnhat, ENT_QUOTES, 'UTF-8');
            $data['phone']      = ! empty($user->phone) ? $user->phone : NULL;
            $data['avatar']      = ! empty($user->avatar) ? base_url().'/uploads/'.$user->username.'/avatar/'.$user->avatar :  base_url().'/uploads/default/avatar.png';
            return $data;
        }else{
